...
bbox_csrt = (box_x_tl, box_y_tl, 200, 200)
tracker.init(frame, bbox_csrt)
# while ret:
while True:
    ret, frame = cap.read()
    ...
    success, bbox_csrt = tracker.update(frame)

    if success:
        # print(bbox)
        x_csrt,y_csrt,w_csrt,h_csrt = int(bbox_csrt[0]),int(bbox_csrt[1]),int(bbox_csrt[2]),int(bbox_csrt[3])
        frame = cv2.rectangle(frame, (x_csrt,y_csrt), (x_csrt+w_csrt, y_csrt+h_csrt), (255,0,0), 3)
    ...
    # #     print(i)
        ...
        #[x0,y1]: top left ; [x2,y3]: bottom right
        w = cords[2] - cords[0]
        h = cords[3] - cords[1]
